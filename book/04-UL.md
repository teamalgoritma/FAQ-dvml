...
Dari output di atas diketahui bahwa variabel numerik sudah berbentuk sebuah PC.

9. Bagaimana penggunaan visualisasi PCA menggunakan package `factoextra`?
...
- `quali.sup`: Nomor kolom dari varaibel kategorik.
...
Setelah membuat PCA, selanjutnaya adalah membuat visualisasi dari hasil PCA. Kita akan membuat individual plot menggunakan fungsi `fviz_pca_ind()` dari library `factoextra`. Parameter yang digunakan adalah:
